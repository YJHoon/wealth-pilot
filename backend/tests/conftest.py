...
async def mock_user(db_session: AsyncSession):
    ...
    await db_session.execute(text("DELETE FROM advisory_positions WHERE user_id = :user_id"), params)
    await db_session.execute(text("DELETE FROM analysis_run_items WHERE run_id IN (SELECT id FROM analysis_runs WHERE user_id = :user_id)"), params)
    await db_session.execute(text("DELETE FROM analysis_runs WHERE user_id = :user_id"), params)
