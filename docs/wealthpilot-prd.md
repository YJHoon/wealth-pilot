...
> 통합 개인 자산 관리 플랫폼
> 작성일: 2026-03-09 | 버전: 2.0 (인터뷰 최종 반영)
...
- 자산 등록/수정/삭제
  ...
  - 현금/예적금: 금액, 은행명, 이율 ⭐ 보유 중
  - 국내주식: 종목명, 종목코드, 수량, 매입가 ⭐ 보유 중
  - 해외주식/ETF: 종목명, 티커, 수량, 매입가, 통화 ⭐ 보유 중
  - 코인: 종목명, 심볼, 수량, 매입가 ⭐ 보유 중
...
GET    /api/security/sessions        # 활성 세션 관리
```
...
│   │   ├── routers/             # API 라우터
│   │   ├── services/            # 비즈니스 로직
│   │   │   ├── price_service.py # 시세 조회 (3모드)
│   │   │   ├── analysis_service.py # 종목 분석 (기본+기술)
│   │   │   ├── news_service.py  # 뉴스 수집/요약
│   │   │   ├── crypto_service.py # 암호화/복호화
│   │   │   ├── alert_service.py # 텔레그램 알림
│   │   │   └── security_service.py # 2FA, 세션, 로그
│   │   ├── tasks/               # 배치 작업 (크론잡)
│   │   ├── middleware/          # 인증, Rate Limit, 로깅
│   │   └── utils/               # 유틸리티
│   ├── alembic/                 # DB 마이그레이션
│   ├── tests/                   # pytest 테스트
│   ├── requirements.txt
...
### Phase 1 MVP 완성 기준
- [ ] Google 로그인 + 2FA(OTP) 동작
- [ ] 30분 미사용 자동 로그아웃 + 기기별 세션 관리
- [ ] 자산 CRUD + 매도 아카이브 동작
- [ ] 포트폴리오 그룹 생성 및 자산 할당
